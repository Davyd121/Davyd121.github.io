...
let currency = "UAH";
    ...
    function changePricing(cur){
        if( cur == "UAH" && currency == "USD"){
            ...
            showOrders(allOrders_2, "UAH");
            const sum = allOrders_2.reduce( function(acc, el){
    
                return acc += el.price
            },0 )
            
            document.getElementById("result").innerText = `Загвлом витрачено - ${sum} UAH `
        }
        if( cur == "USD"  && currency == "UAH" ){
            ...
            document.getElementById("result").innerText = `Загвлом витрачено - ${sum} USD `
